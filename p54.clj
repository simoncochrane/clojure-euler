...
(def player1
     (re-seq #"(?m)^.{14}" hands-str))

(def player2
     (re-seq #"(?m).{14}$" hands-str))

(defn get-hand [hand-str]
  (let [hand-m (re-matcher #"([KQJAT2-9])([CSDH])" hand-str)]
    (loop [hand {}
	   card (re-find hand-m)
	   last-suit nil
	   flush true]
      (if (empty? card)
	(assoc hand :flush flush)
	(let [value (condp = (nth card 1)
		       "T" 10
		       "J" 11
		       "Q" 12
		       "K" 13
		       "A" 14
		       (Integer/parseInt (nth card 1)))
	      suit (nth card 2)]
	  (recur (assoc hand value (inc (get hand value 0)))
		 (re-find hand-m)
		 suit
		 (if (and flush (or (nil? last-suit) (= suit last-suit)))
		   true
		   false)))))))
...
(def *FULL-HOUSE*     6)
(def *FOUR-KIND*      7)
(def *STRAIGHT-FLUSH* 8)
(def *ROYAL-FLUSH*    9))

(defn get-rank
  "Return the ordered rank (0-9 with 9 being royal flush) as the first element
   of a sequence with the following elements representing the card values in order
   appropriate for comparison where rank of two hands is equivalent."
  [hand]
  (let [flush (get hand :flush)]
    (loop [h (dissoc hand :flush)
	   rank *HIGH-CARD*
	   vals-rank-1 []
	   vals-rank-gt1 []]
      (if (empty? h)
	(if (= rank *HIGH-CARD*)
	  (if (straight? vals-rank-1)
	    (if flush
	      (if (= 10 (min vals-rank-1))
		(cons *ROYAL-FLUSH* (rsort vals-rank-1))
		(cons *STRAIGHT-FLUSH* (rsort vals-rank-1)))
	      (cons *STRAIGHT* (rsort vals-rank-1)))
	    (cons (if flush *FLUSH* *HIGH-CARD*) (rsort vals-rank-1)))
	  (cons rank (concat vals-rank-gt1 (rsort vals-rank-1))))
	(let [k (first (first h))]
	  (condp = (get h k)
	      1 (recur (dissoc h k) rank (conj vals-rank-1 k) vals-rank-gt1)
	      2 (condp = rank
		    *HIGH-CARD*  (recur (dissoc h k) *ONE-PAIR*
					vals-rank-1 (conj vals-rank-gt1 k))
		    *ONE-PAIR*   (recur (dissoc h k) *TWO-PAIR*
					vals-rank-1 (rsort (conj vals-rank-gt1 k)))
		    *THREE-KIND* (recur (dissoc h k) *FULL-HOUSE*
					vals-rank-1 (conj vals-rank-gt1 k)))
	      3 (condp = rank
		    *HIGH-CARD* (recur (dissoc h k) *THREE-KIND*
				       vals-rank-1 (conj vals-rank-gt1 k))
		    *ONE-PAIR*  (recur (dissoc h k) *FULL-HOUSE*
				       vals-rank-1 (cons k vals-rank-gt1)))
	      4 (recur (dissoc h k) *FOUR-KIND* vals-rank-1 (conj vals-rank-gt1 k))))))))

(defn eul54[]
  (reduce +
	  (map #(loop [r1 (get-rank (get-hand %1)) r2 (get-rank (get-hand %2))]
		    (cond (> (first r1) (first r2)) 1
			  (< (first r1) (first r2)) 0
